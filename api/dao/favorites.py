...
class FavoriteDAO:
   ...
   *
    ...
    def add(self, user_id, movie_id):
        def add_to_favorites(tx,movie_id,userId):
            row = tx.fun(""" MATCH (u:User {userId: $userId})
                            ...
                            RETURN m {
                                .*,
                                favorite: true
                            } AS movie""" , userId=user_id,movieId=movie_id).single()
            if row == None:
                raise NotFoundException()
            
            return row.get("moivie")
        with self.driver.session() as session:
    ...
    def remove(self, user_id, movie_id):
        def remove_from_favorites(tx,user_id,movie_id):
            res = tx.run(""" MATCH (u:User {userId: $userId})-[r:HAS_FAVORITE]->(m:Movie {tmdbId: $movieId})
                            DELETE r
        ...
        with self.driver.session as session:
            return session.execute_write(remove_from_favorites,user_id=user_id,movie_id=movie_id)
